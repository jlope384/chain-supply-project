<script>
  ...
  const columns = [
    ...
    { key: "active",         label: "Estado",   render: (v) => v
  ...
  async function load() {
    loading = true;
    try { rows = await api.suppliers.list({ limit: 200 }); }
  ...
  function openEdit(row) {
    editing = row;
    form = {
      name: row.name, country: row.country, active: row.active,
      rating: row.rating,
  ...
  async function remove(row) {
    ...
    try { await api.suppliers.remove(row.id); toast("Eliminado", "success"); load(); }
    catch (e) { toast(e.message, "error"); }
  }
</script>
...
<Modal bind:open={showModal} title={editing ? "Editar Proveedor" : "Nuevo Proveedor"}>
  <form onsubmit={(e) => { e.preventDefault(); save(); }} class="form">
    <div class="grid-2">
      ...
      <label>Rating (0–5) <input type="number" step="0.1" min="0" max="5" bind:value={form.rating} /></label>
      <label>Fundado <input type="date" bind:value={form.founded_at} /></label>
    </div>
